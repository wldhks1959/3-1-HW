...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        processes[i].executionTime = executionTime[i];
        processes[i].waitTime = 0;
        processes[i].returnTime = 0;
    }
    qsort(processes, NUM_THREADS, sizeof(Process), compareExecutionTime);
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        pthread_create(&threads[i], NULL, SJF, (void *)process);
        pthread_join(threads[i], NULL);
    }

    // 평균 대기시간 / 반환시간
    double avgWaitTime = totalWaitTime / 5.0;
    double avgReturnTime = totalReturnTime / 5.0;
    printf("평균 대기시간: %.1f\n", avgWaitTime);
    printf("평균 반환시간: %.1f\n", avgReturnTime);
