...
// 체크섬 계산 함수
unsigned short checksum(const char *data) 
{
    unsigned int sum = 0;
...
void* sender(void *args) 
{
    ...
    long timeout_interval = 5000000; // 타임아웃 간격 (5초)
    ...
    while (base < payload_cnt) 
    {
        ...
        {
            unsigned short chk = checksum(payload[nextseqnum]); // 체크섬 계산
            packet[0] = nextseqnum % 10 + '0';  // 패킷 번호

            memcpy(packet + 1, payload[nextseqnum], PAYLOAD_SIZE); 
            memcpy(packet + 1 + PAYLOAD_SIZE, &chk, sizeof(unsigned short)); 
